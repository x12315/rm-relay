...
assert_contains "${versions_file}" 'CATCH2_PACKAGE_VERSION=3.4.0-1build1'
...
assert_contains "${base_smoke}" 'ccache'
assert_contains "${base_smoke}" 'gcc-14 -dumpfullversion'
assert_contains "${base_smoke}" 'mise --version | grep -F "${MISE_VERSION}"'
assert_contains "${base_smoke}" "dpkg-query -W -f='\${Version}' catch2"
assert_contains "${base_smoke}" '/usr/lib/cmake/Catch2/Catch2Config.cmake'
