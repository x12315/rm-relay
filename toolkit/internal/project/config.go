...
// Build declares the mise task and output files for one RM Relay Profile.
type Build struct {
...
func (config Config) validate() error {
	...
	for buildIndex, build := range config.Builds {
		...
		if !isSafeRelativeFile(build.MiseConfig) {
			return fmt.Errorf("build %q mise_config must be a safe relative file", build.Profile)
		}
		if strings.TrimSpace(build.Task) == "" {
			return fmt.Errorf("build %q task must not be empty", build.Profile)
		}
