...
> 战队部署方案、Linux 应用环境、远程构建和 Development Session 尚未交付。路线图和
> 架构文档表达建设方向，不代表已经支持。

RM 队伍的成员和工程经验随赛季快速流动。一套环境如果只能由少数人安装、升级和排错，
很容易在交接后失效。RM Relay 将重复出现的工具链配置、项目入口和验证方法整理成可复现
的公共基础设施。项目的完整边界从开发环境延伸到目标设备上的临时开发会话；用户负责应用
源码和启动方式，平台负责源码以下的构建、传输、调试接入与清理。
...
和虚拟 target 链路。它与本地、战队服务器使用同一任务入口，不建设浏览器 IDE，也不承诺
生产级可用性。公开注册与多租户服务属于后续方向。
...
面向战队运维人员，在战队服务器上组合 Registry、远程 workspace 构建与可选虚拟 target，
